...
void screen_header(unsigned int tx_interval_s, float min_dist_moved, char *cached_sf_name, boolean in_deadzone,
  ...
  if (millis() % 6000 < 3000) {
    // Voltage and Percent
    snprintf(buffer, sizeof(buffer), "%d%% %.2fV", PMU ? PMU->getBatteryPercent() : 0,
             PMU ? PMU->getBattVoltage() / 1000.0 : 0.0);
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    display->drawString(BATTERY_IMAGE_WIDTH, 2, buffer);

    if (PMU->getBatteryPercent() < 10) {
      display->drawXbm(0, 2, BATTERY_IMAGE_WIDTH, BATTERY_IMAGE_HEIGHT, BATTERY_EMPTY_IMAGE);
    } else if (PMU->getBatteryPercent() < 40) {
      display->drawXbm(0, 2, BATTERY_IMAGE_WIDTH, BATTERY_IMAGE_HEIGHT, BATTERY_LOW_IMAGE);
    } else if (PMU->getBatteryPercent() < 70) {
      display->drawXbm(0, 2, BATTERY_IMAGE_WIDTH, BATTERY_IMAGE_HEIGHT, BATTERY_HALF_IMAGE);
    } else {
      display->drawXbm(0, 2, BATTERY_IMAGE_WIDTH, BATTERY_IMAGE_HEIGHT, BATTERY_FULL_IMAGE);
    }
