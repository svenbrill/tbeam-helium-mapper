...
void AppWeb::begin() {
    ...
    server->addHandler(
        new FunctionRequestHandler(
            [](WebServer &server) {
                File dataFile = FFat.open("/www/assets/index-a1346198.js.gz", "rb");
                if (!dataFile) {
                    return ;
                }
    ...
    server->addHandler(
        new FunctionRequestHandler(
            [](WebServer &server) {
                File dataFile = FFat.open("/www/assets/index-d982a335.css.gz", "rb");
                if (!dataFile) {
                    return ;
                }
            ...
            nullptr,
            "/assets/index-d982a335.css",
            HTTP_GET
        )
    );
    server->serveStatic(
        "/assets/Info-548b4797.js",
        FFat,
        "/www/assets/Info-548b4797.js"
    );
    server->serveStatic(
        "/assets/Info-1e11ed16.css",
        FFat,
        "/www/assets/Info-1e11ed16.css"
    );
    server->serveStatic(
        "/assets/Settings-5be5b79d.js",
        FFat,
        "/www/assets/Settings-5be5b79d.js"
    );
    server->serveStatic(
        "/assets/Upgrade-f1d3efb8.js",
        FFat,
        "/www/assets/Upgrade-f1d3efb8.js"
    );
    server->begin();
}
